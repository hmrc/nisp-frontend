/* global $ */
/* eslint-disable no-unused-vars, sonarjs/cognitive-complexity, sonarjs/no-duplicate-string */
...
$(document).ready(function () {
  ...
  $(summary).on('click enter', function () {
    ...
    var professionalAdvice =
      document.getElementById('details-content-0').attributes[2].value;
    var nirSection =
      document.getElementById('details-content-3').attributes[2].value;
  ...
  var currentLanguage = $('#current-language').text().trim();

  $('.hide-details').hide();

  $('.accordion > dt.expandable').click(function () {
    var taxYear = $(this).find('.ni-years').text();
    var showDetailsEnglish =
      'View <span class="govuk-visually-hidden">' +
      taxYear +
      '</span>details';
    var showDetailsWelsh =
      'Gweld y manylion<span class="govuk-visually-hidden">' +
      taxYear +
      '</span>';

    var hideDetailsEnglish =
      'Hide <span class="govuk-visually-hidden">' +
      taxYear +
      '</span>details';
    var hideDetailsWelsh =
      'Cuddio manylion<span class="govuk-visually-hidden">' +
      taxYear +
      '</span>';
