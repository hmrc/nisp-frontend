...
class PertaxAuthConnectorImpl @Inject()(
  ...
  override def loadPartial(partialContextUrl: String)(implicit hc: HeaderCarrier): Future[HtmlPartial] = {
    val partialPath: String = if (partialContextUrl.startsWith("/")) partialContextUrl else s"/$partialContextUrl"
    val partialUrl =  appConfig.pertaxAuthBaseUrl + partialPath
    http
      .get(url"${partialUrl}")
      .execute[HtmlPartial]
      .map {
