...
import uk.gov.hmrc.nisp.helpers.MockFullNIRecordController
import uk.gov.hmrc.play.frontend.auth.AuthenticationProviderIds
import uk.gov.hmrc.play.http.SessionKeys
import uk.gov.hmrc.play.test.UnitSpec
import uk.gov.hmrc.time.DateTimeUtils._

class FullNIRecordControllerSpec extends UnitSpec with OneAppPerSuite {
  ...
  "GET /account/nirecord/gaps (gaps)" should {
    "return redirect for unauthenticated user" in {
      val result = MockFullNIRecordController.showGaps(fakeRequest)
      redirectLocation(result) shouldBe Some(ggSignInUrl)
    }

    "return gaps page for user with gaps" in {
      val result = MockFullNIRecordController.showGaps(authenticatedFakeRequest(mockUserId))
      contentAsString(result) should include ("Years which are not full")
    }

    "return full page for user without gaps" in {
      val result = MockFullNIRecordController.showGaps(authenticatedFakeRequest(mockFullUserId))
    ...
    "return error page for blank response NINO" in {
      intercept[RuntimeException] {
        val result = MockFullNIRecordController.showGaps(authenticatedFakeRequest(mockBlankUserId))
        status(result) shouldBe Status.INTERNAL_SERVER_ERROR
      }
    }

    "redirect to exclusion for excluded user" in {
      val result =  MockFullNIRecordController.showGaps(fakeRequest.withSession(
        SessionKeys.sessionId -> s"session-${UUID.randomUUID()}",
        SessionKeys.lastRequestTimestamp -> now.getMillis.toString,
  ...
  "GET /account/nirecord (full)" should {
    "return redirect for unauthenticated user" in {
      val result = MockFullNIRecordController.showFull(fakeRequest)
      redirectLocation(result) shouldBe Some(ggSignInUrl)
    }

    "return gaps page for user with gaps" in {
      val result = MockFullNIRecordController.showFull(authenticatedFakeRequest(mockUserId))
      contentAsString(result) should include ("All years.")
    }

    "return full page for user without gaps" in {
      val result = MockFullNIRecordController.showFull(authenticatedFakeRequest(mockFullUserId))
      contentAsString(result) should include ("You do not have any gaps in your record.")
    }

    "return NI record page with link to full years" in {
      val result = MockFullNIRecordController.showFull(authenticatedFakeRequest(mockFullUserId))
      contentAsString(result) should include ("&pound;896.64")
    }

    "redirect to exclusion for excluded user" in {
      val result =  MockFullNIRecordController.showFull(fakeRequest.withSession(
        SessionKeys.sessionId -> s"session-${UUID.randomUUID()}",
        SessionKeys.lastRequestTimestamp -> now.getMillis.toString,
        SessionKeys.userId -> mockUserIdExcluded,
        SessionKeys.authProvider -> AuthenticationProviderIds.VerifyProviderId
  ...
  "GET /account/nirecord/gapsandhowtocheck" should {
    "return redirect for unauthenticated user" in {
      val result = MockFullNIRecordController.showGapsAndHowToCheckThem(fakeRequest)
      redirectLocation(result) shouldBe Some(ggSignInUrl)
    }

    "return how to check page for authenticated user" in {
      val result = MockFullNIRecordController.showGapsAndHowToCheckThem(authenticatedFakeRequest(mockUserId))
  ...
  "GET /account/nirecord/voluntarycontribs" should {
    "return redirect for unauthenticated user" in {
      val result = MockFullNIRecordController.showVoluntaryContributions(fakeRequest)
      redirectLocation(result) shouldBe Some(ggSignInUrl)
    }

    "return how to check page for authenticated user" in {
      val result = MockFullNIRecordController.showVoluntaryContributions(authenticatedFakeRequest(mockUserId))
