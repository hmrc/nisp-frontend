...
object FakeTemplateRenderer extends TemplateRenderer {
  ...
  def displayUrBanner(extraArgs: Map[String, Any]): String =
    if (extraArgs.contains("fullWidthBannerTitle")) {
      "<div id=full-width-banner>" + "<div class = \"full-width-banner__title\">" + extraArgs(
        "fullWidthBannerTitle"
      ) + "</div>" + "<div id = fullWidthBannerLink>" + extraArgs(
        "fullWidthBannerLink"
      ) + "</div>" + "<div>" + extraArgs(
        "fullWidthBannerText"
      ) + "</div>" + "<div id = fullWidthBannerDismissText>" + extraArgs("fullWidthBannerDismissText") + "</div>"
    } else ""
